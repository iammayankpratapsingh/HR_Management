...
const AddEmployee = () => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('http://localhost:3002/auth/add_employee', formData)
      .then(result => {
        if (result.data.Status) {
      ...
      .catch(err => {
        console.error(err);
        alert('An error occurred while adding the employee. Please try again.'); // User-friendly error message
      });
  };
        ...
        <form className="row g-1" onSubmit={handleSubmit}>
          ...
          <div className="col-12">
            ...
            >
              ...
              {category.map((c) => (
                <option key={c.id} value={c.id}>{c.name}</option> // Added key for unique identifier
              ))}
            </select>
          </div>
